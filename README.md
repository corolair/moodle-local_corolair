...
**Releases.** `main` is generated rather than merged into. Every push to `develop` runs the *Release to main* workflow, which branches from `main`, replaces the tree with `develop`'s, applies the transform above, and opens a pull request from `release/next`. CI runs the full suite against that branch — the production configuration, exercised as production — and `main` is protected so that pull request is the only way in. Nothing merges on its own; the merge stays a human decision.
